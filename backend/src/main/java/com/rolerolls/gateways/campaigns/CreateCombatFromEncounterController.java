...
public class CreateCombatFromEncounterController  {
    ...
    public @ResponseBody
    Combat create(@PathVariable UUID campaignId, @RequestBody CreateCombatFromEncounterInput input) {
        Encounter encounter = encounterRepository.findById(input.encounterId).get();

        Combat combat = combatFromEncounterBuilder.BuildFromEncounter(campaignId, encounter);
